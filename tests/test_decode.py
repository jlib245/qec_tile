...
from qec_tile.decode import logical_error_rate, make_decoder, sample_residuals
...
def test_zero_noise_never_fails():
    assert logical_error_rate(paper_code(*SMALL), p=0.0, shots=200, seed=0) == 0.0
...
def test_correction_always_matches_syndrome():
    ...
    for residual in sample_residuals(code, p=0.08, shots=100, seed=1):
...
def test_rate_grows_with_p():
    code = paper_code(*SMALL)
    low = logical_error_rate(code, p=0.02, shots=800, seed=2)
    high = logical_error_rate(code, p=0.12, shots=800, seed=2)
    assert low < high


def test_seed_is_deterministic():
    code = paper_code(*SMALL)
    a = logical_error_rate(code, p=0.08, shots=300, seed=7)
    b = logical_error_rate(code, p=0.08, shots=300, seed=7)
    assert a == b


def test_rate_is_a_fraction():
    rate = logical_error_rate(paper_code(*SMALL), p=0.1, shots=500, seed=3)
    assert 0.0 <= rate <= 1.0
