"""Min-sum belief propagation — cross-checked against ldpc, iteration by
iteration.
...
so the two stay the same decoder.
"""
import numpy as np
import pytest
from ldpc import BpDecoder

from qec_tile.bp import min_sum, min_sum_trace
...
def test_llr_matches_ldpc_each_iteration(name, qubits):
...
def test_zero_syndrome_never_flags_a_qubit():
    ...
    prior = np.log((1 - P) / P)
    steps = list(min_sum_trace(code.HZ, syndrome, P, max_iter=4))
...
def test_channel_accepts_a_vector():
    ...
    flat = list(min_sum_trace(code.HZ, syndrome, np.full(code.n, P),
                              max_iter=4))
    scalar = list(min_sum_trace(code.HZ, syndrome, P, max_iter=4))
