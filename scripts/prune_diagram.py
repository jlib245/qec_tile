...
``optimise_routing``이 낸 최종 배치를 그린다 -- route window shortening(check 이동 ->
단수 조건 흡수)과 trace pruning(가장자리 routing 삭제)을 거치고, data-start shift를
켠/끈 두 후보 중 작은 쪽.

사용법:
    python scripts/routing_diagram.py --word N2ESEN2 --size 4x4
"""
...
from qec_tile.walk2 import (CHECK_X, CHECK_Z, DATA, ROUTING, Qubit,
                            optimise_routing, round_births, walk_layout)
...
def figure(code, word: str, scale: float = 24.0) -> str:
    ...
    after, starts = optimise_routing(code, word)
...
def main():
    ...
    stem = os.path.join(args.out_dir, f"{args.word}_{M}x{N}_routing")
